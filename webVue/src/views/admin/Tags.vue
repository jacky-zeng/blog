  ...
  <div class="tags-page">
    <el-card>
      ...
      <el-table :data="tags" v-loading="loading" style="width: 100%">
        ...
        <el-table-column prop="articles_count" label="使用次数" width="120" />
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      loading.value = true
      try {
        const url = isEdit.value ? `/admin/tags/${currentId.value}` : '/admin/tags'
...
const handleDelete = async (row) => {
  try {
    ...
    const res = await request.delete(`/admin/tags/${row.id}`)
    if (res.code === 200) {
